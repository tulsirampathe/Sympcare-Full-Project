...
const ChatbotButton = ({ isOpen, toggleChat }) => {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      className={`fixed top-52 right-6 bg-white shadow-xl border border-gray-200 rounded-2xl p-4 w-64 flex items-center space-x-4 hover:shadow-2xl transition-all duration-300 z-40 ${
        isOpen ? "hidden" : "block"
      }`}
      aria-label="Open Drug Intraction Chat"
    >
...
// Main Chat Component
const DrugIntraction = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [isMaximized, setIsMaximized] = useState(false);
  const [messages, setMessages] = useState([
    {
      sender: "bot",
      text: "👋 Welcome to SympCare - Your Healthcare Drug Intraction Assistant!",
    },
        ...
        <div
          className={`${
            isMaximized
              ...
              : "fixed top-20 right-28 flex justify-start items-start z-50"
          }`}
        >
          <motion.div
